(load-relative "Disable/disable-mode.el")

(load-relative "Enable/enable-mode.el")

;; (load-relative "God-Mode/god-mode.el")

(load-relative "CC/cc.el")

(load-relative "Ediff/ediff.el")

(load-relative "Exwm/exwm.el")

(load-relative "Gnus/gnus.el")

(load-relative "Goto-last-point/goto-last-point.el")

;; (load-relative "Hammy/hammy.el")

(load-relative "Org/org.el")

;;(load-relative "Polyp/polyp.el")

;; (load-relative "Ryo/ryo.el")

(load-relative "Tmr/tmr.el")

(load-relative "Smartparens/smartparens.el")
